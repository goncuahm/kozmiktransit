...
SIGN_PLANETS  = ["mercury", "venus", "mars"]
...
st.markdown(
    ...
    .aspect-row-bull { background:#0a1a0f; border-left:3px solid #44DD88; padding:6px 12px; margin:2px 0; border-radius:0 4px 4px 0; font-size:13px; }
    .aspect-row-bear { background:#1a0a0f; border-left:3px solid #FF4466; padding:6px 12px; margin:2px 0; border-radius:0 4px 4px 0; font-size:13px; }
    .forecast-box {
    ...
    .gold-text   { color:#C8A84B; }
    .teal-text   { color:#00D4B4; }
...
def valid_pairs(avail):
    planet_list = [p for p in ALL_PLANETS if p in avail]
    pairs, seen = [], set()
...
def build_sign_dummies(eph_slice, feat_cols):
...
def build_features(date_index, eph, orb_apply, orb_sep):
    eph_al = eph.reindex(date_index, method="ffill")
    avail  = [p for p in ALL_PLANETS if p in eph_al.columns]
    pairs  = valid_pairs(avail)
    ...
    feat_cols = build_sign_dummies(eph_al, feat_cols)
...
def build_forecast_features(fut_dates, eph, feature_cols, orb_apply, orb_sep):
    ...
    avail    = [p for p in ALL_PLANETS if p in eph_fut.columns]
    pairs    = valid_pairs(avail)
    all_used = sorted(set(p for pair in pairs for p in pair))
    ...
    feat_cols_d = build_sign_dummies(eph_fut, feat_cols_d)
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("<h2 style='text-align:center;margin-bottom:4px'>🪐 Planetary</h2>", unsafe_allow_html=True)
    st.markdown("<h2 style='text-align:center;margin-top:0'>Regression</h2>", unsafe_allow_html=True)
    st.markdown("<hr class='gold'/>", unsafe_allow_html=True)

    st.markdown("### 📁 Ephemeris Data")
    uploaded = st.file_uploader(
        "Upload planet_degrees.csv",
        type=["csv"],
        help="CSV with 'date' index and planet columns (sun, moon, mercury …)",
    )

    st.markdown("<hr class='gold'/>", unsafe_allow_html=True)
    st.markdown("### 📈 Stock Settings")
    ticker = st.text_input("Stock Ticker", value="GC=F",
                           help="Yahoo Finance ticker, e.g. GC=F, SI=F, AAPL, BTC-USD")
    stock_name = st.text_input("Display Name", value="Gold Futures",
                               help="Used in chart titles and labels")

    today       = datetime.date.today()
    default_end = today + datetime.timedelta(days=365)

    start_date = st.date_input("Data Start Date", value=datetime.date(2000, 1, 1),
                               min_value=datetime.date(1950, 1, 1),
                               max_value=today)
    forecast_end = st.date_input("Forecast End Date",
                                 value=default_end,
...
# ══════════════════════════════════════════════════════════════════════════════
st.markdown(
    f"<h1 style='text-align:center;letter-spacing:3px;margin-bottom:4px'>"
    f"PLANETARY REGRESSION</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    ...
    st.markdown(
        ...
        <div style='text-align:center;padding:60px 20px;color:#555'>
            ...
            <div style='font-family:Cinzel,serif;font-size:20px;color:#C8A84B;margin-bottom:12px'>
                Upload Your Ephemeris to Begin
            </div>
            <div style='font-size:14px;line-height:1.8'>
                1. Upload <code>planet_degrees.csv</code> in the sidebar<br>
                2. Set your stock ticker and date range<br>
                3. Adjust orb angles<br>
                4. Click <strong>Run Analysis</strong>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with st.spinner("Loading ephemeris …"):
    try:
        eph = pd.read_csv(uploaded, index_col="date", parse_dates=True)
        st.success(f"✓ Ephemeris loaded: {eph.shape[0]} rows, "
                   f"{eph.shape[1]} planets  ({eph.index[0].date()} → {eph.index[-1].date()})")
...
with st.spinner(f"Downloading {ticker} from Yahoo Finance …"):
    try:
        ...
        raw = yf.download(ticker, start=start_date.strftime("%Y-%m-%d"),
                          end=stock_end_str, progress=False, auto_adjust=False)
        ...
        st.success(f"✓ {ticker} downloaded: {len(raw)} trading days "
...
with st.spinner("Building feature matrix …"):
    ...
    pairs_used   = valid_pairs([p for p in ALL_PLANETS if p in eph.columns])
...
st.markdown(
    ...
    f"<span class='metric-sub'>({n_asp_cols} aspect + {n_sign_cols} sign dummies) · "
    f"{len(pairs_used)} valid planet pairs · "
    f"Apply orb {orb_apply}° / Sep orb {orb_sep}°</span></div>",
    unsafe_allow_html=True,
)
...
with st.spinner("Generating 1-year forecast …"):
    ...
    fut_dates      = pd.date_range(start=forecast_start,
                                   end=forecast_end.strftime("%Y-%m-%d"), freq="B")
    n_fut          = len(fut_dates)

    X_fut        = build_forecast_features(fut_dates, eph, feature_cols, orb_apply, orb_sep)
...
c5.markdown(metric_html("Up Days (Logit)", f"{n_up}", f"{n_up/n_fut*100:.1f}% of {n_fut} days"), unsafe_allow_html=True)
c6.markdown(metric_html("Down Days (Logit)", f"{n_down}", f"{n_down/n_fut*100:.1f}% of {n_fut} days"), unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
#  PLOT 1 — Last 12M + 3-Month forecast
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("---")
...
ols_fit_ly      = cumret_ols_fit[mask_ly]
log_fit_ly      = cumret_log_fit[mask_ly]

fore_dates_3m = fut_dates[:TRADING_MONTH]
fore_ols_3m   = fore_ols_cum[:TRADING_MONTH]
fore_log_3m   = fore_log_price[:TRADING_MONTH]
y_top_val     = max(actual_ly.max(), ols_fit_ly.max(), log_fit_ly.max(),
                    fore_ols_3m.max() if len(fore_ols_3m) else 0,
                    fore_log_3m.max() if len(fore_log_3m) else 0)
...
ax.plot(dates_ly, ols_fit_ly, color=ORANGE, lw=1.5, alpha=0.85, zorder=4, label="OLS fitted")
ax.plot(dates_ly, log_fit_ly, color=GREEN,  lw=1.5, alpha=0.85, zorder=4, label="Logistic fitted")
...
if len(fore_dates_3m):
    ...
    ax.plot([dates_ly[-1], fore_dates_3m[0]], [last_ols_fit, fore_ols_3m[0]],
            color=ORANGE, lw=1.2, alpha=0.7, zorder=4)
    ax.plot(fore_dates_3m, fore_ols_3m, color=ORANGE, lw=2.0, ls="--", zorder=5,
            label=f"OLS 3M  end={fore_ols_3m[-1]:,.4f}  ({(fore_ols_3m[-1]/last_actual-1)*100:+.1f}%)")
    ...
    ax.plot([dates_ly[-1], fore_dates_3m[0]], [last_log_fit, fore_log_3m[0]],
            color=GREEN, lw=1.2, alpha=0.7, zorder=4)
    ax.plot(fore_dates_3m, fore_log_3m, color=GREEN, lw=2.0, ls="--", zorder=5,
            label=f"Logistic 3M  end={fore_log_3m[-1]:,.4f}  ({(fore_log_3m[-1]/last_actual-1)*100:+.1f}%)")
...
if len(fore_dates_90):
    ...
    ax.plot(fore_dates_90, fore_ols_90, color=ORANGE, lw=2.4, ls="--", zorder=5,
            label=f"OLS {FORECAST_DAYS}d  end={fore_ols_90[-1]:,.4f}  ({(fore_ols_90[-1]/last_actual-1)*100:+.2f}%)")
    ...
    for i in peak_idx:
        ...
        ax.text(d, lyt, f"  ▲ {d.strftime('%b %d')}\n  {v:,.2f}",
                ...
                bbox=dict(boxstyle="round,pad=0.2", facecolor="#1a0010", edgecolor="#FF4466", alpha=0.85))
    ...
    for i in trough_idx:
        ...
        ax.text(d, lyb, f"  ▼ {d.strftime('%b %d')}\n  {v:,.2f}",
                ...
                bbox=dict(boxstyle="round,pad=0.2", facecolor="#001a0a", edgecolor="#44FF88", alpha=0.85))

    ax.set_ylim(y_min_ax, y_max_ax)

ax.scatter([dates_6m[-1]], [last_actual], color=TEAL, s=80, zorder=8, label=f"Last: {last_actual:,.4f}")
...
ax.set_title(
    ...
    f"Pairs: {len(pairs_used)}  ·  Features: {n_avail}  ·  Orb: apply={orb_apply}°/sep={orb_sep}°",
    color=GOLD, fontsize=11, fontweight="bold")
...
ax.set_title(
    ...
    f"End: {fore_ols_cum[-1]:,.4f}  ({chg_pct:+.2f}% vs last)  ·  Features: {n_avail}",
    color=GOLD, fontsize=11, fontweight="bold")
...
ax_top.annotate(f"{fore_log_price[-1]:,.4f}",
...
ax_top.legend(handles=[up_patch, down_patch] + ax_top.get_legend_handles_labels()[0],
...
with st.spinner("Computing active aspects …"):
    ...
    fut_dates_w  = pd.date_range(start=last_date + pd.Timedelta(days=1),
                                 end=window_end, freq="B")
    all_win_d = past_dates_w.append(fut_dates_w)

    eph_win  = eph.reindex(all_win_d, method="ffill")
    avail_w  = [p for p in ALL_PLANETS if p in eph_win.columns]
    pairs_w  = valid_pairs(avail_w)
...
if len(asp_table):
    ...
    def style_aspect_df(df):
        def row_color(row):
            ...
            return ["background-color:" + color] * (len(row) - 1) + [f"color:{dir_c};background-color:{color}"]
...
for planet in SIGN_PLANETS:
    ...
    sign_rows.append({
        "Planet": planet.capitalize(), "Sign": sign,
        "Longitude °": round(lon, 2), "OLS Coef": round(coef, 6),
        "Effect %": round(coef * 100, 3), "p-value": round(pval, 4),
        "Bias": bias,
    })
...
c2.markdown(
    metric_html("Logit Hit Rate (last 10d)", f"{log_hits}/{n_h}", f"{log_hits/n_h*100:.0f}%"),
    unsafe_allow_html=True,
)
