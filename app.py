...
# Constant step size for ALL logistic up/down moves (fitted history, Chart 1,
# and the forecast) -- using the SAME scalar everywhere means a predicted
# "up" day always moves the price by exactly this much, and a predicted
# "down" day by exactly this much in the other direction, regardless of how
# volatile that particular historical day happened to be. Previously the
# fitted/history paths used that day's ACTUAL |return| as the step size,
# so a few genuinely volatile historical days could produce much larger
# jumps than others, which is what was producing the extreme-looking swings.
...
log_step       = y_fit_log_full * mean_abs_ret
