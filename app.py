...
# ── Logistic: cumulate signed steps within the window, calibrated so the
# fitted path starts AND ends on the actual price (start = actual_ly[0],
# end = actual_ly[-1] = last_actual). The classifier only supplies a
# direction each day, so the *shape* of the path comes from those daily
# up/down calls; a single scale factor (log_scale) stretches or shrinks
# that shape so its total move matches the real total move over the window.
log_dir_win    = y_fit_log_full[window_start_pos:]
raw_step_win   = log_dir_win * np.abs(y_ret[window_start_pos:])
raw_cum_win    = np.cumsum(raw_step_win) - raw_step_win[0]      # day0 baseline = 0
target_growth  = np.log(actual_ly[-1] / actual_ly[0])           # actual log-return, start->end
raw_growth_end = raw_cum_win[-1]

if abs(raw_growth_end) > 1e-9:
    log_scale   = target_growth / raw_growth_end
    log_cum_win = raw_cum_win * log_scale
else:
    # Up/down calls net out to ~zero over the window — no shape to rescale.
    # Fall back to a straight line between the two actual endpoints.
    log_scale   = 1.0
    log_cum_win = np.linspace(0.0, target_growth, len(raw_cum_win))

log_fit_ly = actual_ly[0] * np.exp(log_cum_win)

# Endpoints of the rebased fitted series (used to connect to forecast lines)
last_ols_fit_ly = float(ols_fit_ly[-1])
last_log_fit_ly = float(log_fit_ly[-1])   # == last_actual by construction

fore_dates_3m = fut_dates[:TRADING_MONTH]
fore_ols_3m   = fore_ols_cum[:TRADING_MONTH]

# Logistic forecast: continue from last_actual using the SAME step size —
# mean_abs_ret scaled by the same log_scale factor used to calibrate the
# fitted window — so the fitted line and forecast line sit on one consistent
# scale instead of jumping between two different step conventions.
fore_log_3m_step = y_fore_log[:TRADING_MONTH] * mean_abs_ret * log_scale
fore_log_3m      = last_actual * np.exp(np.cumsum(fore_log_3m_step))
...
ax.plot(dates_ly, log_fit_ly, color=GREEN,  lw=1.5, alpha=0.85, zorder=4,
        label="Logistic fitted (calibrated to window start & end)")
