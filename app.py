...
with st.sidebar:
    ...
    st.markdown("### 📐 Statistical Controls")
    include_lag = st.checkbox(
        "Include yesterday's return (AR(1) baseline)",
        value=False,
        help=(
            "Adds yesterday's log RETURN as an extra feature (not yesterday's "
            "raw PRICE -- price levels trend over time and are non-stationary, "
            "which would risk spurious fits; returns are already close to "
            "stationary on their own, which is also why this app models "
            "returns rather than price levels in the first place). Checking "
            "this tests whether the astrology features add anything beyond a "
            "simple momentum/mean-reversion baseline. The forecast becomes an "
            "iterative (chained) day-by-day prediction when this is enabled, "
            "since each day's forecast then depends on the previous day's "
            "predicted return."
        ),
    )
...
# When the AR(1) lag feature is enabled, every array needs to be trimmed by
# one row (day 0 has no prior return to lag), done ONCE here so everything
# downstream (feature building, fitting, plotting, CSV export) stays aligned
# without special-casing later.
if include_lag:
    y_ret_lag_full = np.concatenate([[np.nan], y_ret[:-1]])
    _valid = ~np.isnan(y_ret_lag_full)
    y_ret_lag = y_ret_lag_full[_valid]
    dates_all = dates_all[_valid]
    y_ret     = y_ret[_valid]
    y_dir     = y_dir[_valid]
    close_all = close_all[_valid]
else:
    y_ret_lag = None
...
with st.spinner("Building feature matrix …"):
    feat_all     = build_features(dates_all, eph, orb_apply, orb_sep,
                                  inner_planets, sign_planets)
    feature_cols = feat_all.columns.tolist()   # astro-only columns (aspects + signs)
    ...
    X_astro_raw = feat_all.values

    # Lag column appended AFTER the astro columns, kept separate from
    # `feature_cols` (which build_forecast_features reindexes against) --
    # the forecast for this column is handled explicitly below via chaining,
    # not by build_forecast_features (which only knows how to compute
    # ephemeris-derived aspect/sign features for future dates).
    if include_lag:
        X_raw = np.hstack([X_astro_raw, y_ret_lag.reshape(-1, 1)])
    else:
        X_raw = X_astro_raw
    X_con = np.hstack([np.ones((len(X_raw), 1)), X_raw])
    col_names_ols = ["const"] + feature_cols + (["lag_return"] if include_lag else [])

st.markdown(
    f"<div class='metric-card'><span class='metric-label'>Features built</span><br>"
    f"<span class='metric-value'>{n_avail}{' + 1 lag' if include_lag else ''}</span> "
    f"<span class='metric-sub'>"
    f"{n_asp_cols} aspect + {n_sign_cols} sign dummies"
    f"{' + 1 AR(1) lag-return feature' if include_lag else ''} · "
...
with st.spinner("Generating forecast …"):
    ...
    # Astro-only forecast features (aspects + signs) -- same regardless of
    # whether the lag feature is on, since build_forecast_features only
    # knows how to compute ephemeris-derived features for future dates.
    ...
    if not include_lag:
        # Original vectorized path -- unchanged.
        ...
        # Chained (iterative) forecast: each day's lag-return input is the
        # PREVIOUS day's prediction, so days must be predicted in order,
        # one at a time, rather than in one matrix multiply. Day 1's lag
        # input is the last actually observed return.
        y_fore_ols_r = np.zeros(n_fut)
        y_fore_log   = np.zeros(n_fut, dtype=int)

        current_lag_ols = y_ret[-1]
        for k in range(n_fut):
            x_row = np.concatenate([[1.0], X_fut[k], [current_lag_ols]])
            pred_r = float(ols_params @ x_row)
            y_fore_ols_r[k] = pred_r
            current_lag_ols = pred_r
        fore_ols_cum = last_actual * np.exp(np.cumsum(y_fore_ols_r))

        current_lag_log = y_ret[-1]
        for k in range(n_fut):
            x_row    = np.concatenate([X_fut[k], [current_lag_log]])
            x_row_sc = sc_full.transform(x_row.reshape(1, -1))
            pred_dir = int(log_full.predict(x_row_sc)[0])
            y_fore_log[k] = pred_dir
            current_lag_log = pred_dir * mean_abs_ret  # proxy return magnitude, for chaining only
...
if include_lag:
    lag_idx = col_names_ols.index("lag_return")
    lag_coef = float(ols_full.params[lag_idx])
    lag_pval = float(ols_full.pvalues[lag_idx])
    st.caption(
        f"ℹ️ AR(1) lag-return coefficient: {lag_coef:+.4f} (p={lag_pval:.4f}). "
        f"Forecast is chained day-by-day since each day depends on the previous "
        f"day's prediction (slower than the no-lag path, especially for long "
        f"forecast horizons)."
    )
...
#
#  Equal-size up/down steps, LOCAL TO THIS PLOT ONLY: fore_log_price_p4 is
#  computed here with a single constant magnitude (mean_abs_ret) for every
#  step, so a predicted "up" day always moves the same amount as a predicted
#  "down" day, regardless of that day's model output scale. This is a
#  plot-local variable -- it does NOT replace the shared `fore_log_price`
#  used by Chart 1's forecast connector/line, which is left untouched.
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("---")
...
# Plot-4-only equal-size series: every up step is exactly +mean_abs_ret,
# every down step exactly -mean_abs_ret. Scoped to this plot via its own
# variable name so nothing else on the page is affected.
mean_abs_ret_p4   = np.abs(y_ret).mean()
fore_log_price_p4 = last_actual * np.exp(np.cumsum(y_fore_log * mean_abs_ret_p4))
...
ax_top.plot(fut_dates, fore_log_price_p4, color=WHITE, lw=2.2, zorder=5,
            label="Logistic cum. direction (equal-size steps)")
for i in range(1, n_fut):
    if y_fore_log[i] != y_fore_log[i - 1]:
        ax_top.axvline(fut_dates[i], color=GOLD, lw=0.8, ls="--", alpha=0.55, zorder=4)
ax_top.scatter([fut_dates[0]],  [fore_log_price_p4[0]],  color=TEAL,  s=80, zorder=8)
ax_top.scatter([fut_dates[-1]], [fore_log_price_p4[-1]], color=WHITE, s=80, zorder=8)
ax_top.annotate(
    f"{fore_log_price_p4[-1]:,.4f}",
    xy=(fut_dates[-1], fore_log_price_p4[-1]),
    xytext=(8, 4), textcoords="offset points",
